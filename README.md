...
| 環境変数 | 説明 | デフォルト値 | 例 |
|---------|------|------------|-----|
| `CHROMECAST_NAME` | 制御対象のChromecastデバイス名<br>ネットワーク上で表示される名前を指定 | `"Dell"` | `"リビングのテレビ"` |
| `STEP` | 音量を下げるステップ幅（負の値）<br>-0.04 = 4%ずつ下げる | `-0.04` | `-0.05` |
| `MIN_LEVEL` | 最小音量レベル（0.0～1.0）<br>この値に達するとスタンバイモードに移行 | `0.3` | `0.2` |
| `INTERVAL_SEC` | 音量調整の間隔（秒）<br>1200秒 = 20分 | `1200` | `600` |
| `DEFAULT_VOLUME` | 最小音量到達後に設定する音量<br>次回起動時の初期音量として使用 | `0.5` | `0.4` |
